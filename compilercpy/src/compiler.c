...
int ex(node *p)
{
	...
	case typeOpr:
		...
		case '=':
		{
			...
			update((p->opr.op[0]->id.id), ex(p->opr.op[1]));
			//printf("success!\n");
			return 0;
		}
		case PRINT:
...
int fetch_var_ar(char *name, int ar_index)
{
	...
	{
		index = symReadL(name);
		if (index>=0 && saveTab[index].declared > 1)
		{
...
void readinto (varItemtype * var)
{
	int temp;
	scanf("%d",&temp);
	if(var->length == 0)
	{
		update(var->name,temp);
	}
	else
	{
		update_arr(var->name,var->length,temp);
	}
}